 ...
 *      3. Deploys the `LMPVaultFactory`(using previous vault template and the 'weth-vault' type).
 *      4. Deploys the `LMPVaultRouter`.
 *      5. Grants the `LMPVaultFactory` the `REGISTRY_UPDATER` role.
 *  This script can be rerun to replace the currently deployed weth-vault LMP vault factory in the system.
 */
